...
class Data_Piratebay extends DataUpstream {
	...
	private function getComments($torrentId) {
		$url = 'http://thepiratebay.pe/ajax_details_comments.php';
		$post = array(
			'id' => $torrentId,
			'page' => '1'
		);
	...
	private function getFileListing($torrentId) {
		$url = 'http://thepiratebay.pe/ajax_details_filelist.php?id='. $torrentId;
	...
	private function buildUrl($query, $page = 0, $sortField = self::SORT_SEEDS, $sortOrder = self::SORT_DESC) {
		...
		$url = 'http://thepiratebay.pe/search/' . urlencode($query) . '/' . $page . '/' . $sortValue;
		return $url;
	}
}
